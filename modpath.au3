...
if $cmdline[0] < 2 then
	msgbox(0, "Modify Path", "Usage:" & @CRLF & "modpath.exe {/add | /del} <path>")
	exit
endif

if $cmdline[1] = "/add" or $cmdline[1] = "/del" then
else
	msgbox(0, "Modify Path", "Usage:" & @CRLF & "modpath.exe {/add | /del} <path>")
	exit
endif
...
if @OSType == "WIN32_WINDOWS" then
	...
	$dir = filegetshortname($cmdline[2])
	...
	if $cmdline[1] = "/add" then
		...
		filewriteline($autoexec, @CRLF & "SET PATH=%PATH%;" & $dir)
		fileclose($autoexec)
...
elseif @OSType == "WIN32_NT" then
	...
	if $cmdline[1] = "/add" then $newpath = $newpath & ";" & $cmdline[2]
